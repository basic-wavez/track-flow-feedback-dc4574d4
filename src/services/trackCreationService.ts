...
import { extractTrackName } from "@/lib/audioUtils";
...
export const uploadTrack = async (
  ...
  try {
    ...
    if (userError) {
      console.error("trackCreationService - Auth error:", userError);
      throw new Error("Authentication error: " + userError.message);
    }
    
    if (!user) {
      console.error("trackCreationService - No authenticated user found");
      throw new Error("You must be logged in to upload tracks");
    }

    console.log("trackCreationService - Authenticated user:", user.id);
    ...
    console.log("trackCreationService - Generated unique path:", filePath);
    
    // Upload file to storage with progress tracking
    console.log("trackCreationService - Starting file upload");
    ...
    if (parentTrackId && !explicitVersionNumber) {
      ...
      if (parentTrackError) {
        console.error("trackCreationService - Error fetching parent track:", parentTrackError);
      } else if (parentTrack) {
        // Increment the version number
        versionNumber = (parentTrack.version_number || 1) + 1;
        console.log("trackCreationService - New version number:", versionNumber);
    ...
    console.log("trackCreationService - Creating track record with title:", trackTitle);
    ...
    if (trackError) {
      console.error("trackCreationService - Error creating track record:", trackError);
      throw trackError;
    }

    console.log("trackCreationService - Track record created successfully:", track.id);

    // Automatically request MP3 processing for the new track
    if (track) {
      requestMp3Processing(track.id)
        .then(() => console.log("trackCreationService - MP3 processing requested for track", track.id))
        .catch(error => console.error("trackCreationService - Failed to request MP3 processing:", error));
    }
    
    return track;
  } catch (error: any) {
    console.error("trackCreationService - Upload error:", error);
